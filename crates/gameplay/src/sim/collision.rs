use data::geometry::{Footprint, Position};
use data::math::Scalar;
use world::Map;

pub fn resolve_collisions(ecs: &mut hecs::World, map: &Map) {
    let half = Scalar::const_from_int(1).strict_div_int(2);
    let mut units: Vec<(hecs::Entity, Position, Footprint)> = ecs
    ...
    for _ in 0..3 {
        ...
        for i in 0..units.len() {
            for j in (i + 1)..units.len() {
                ...
                if map.is_area_passable(units[i].2.disc_at(new_a)) {
                    units[i].1 = new_a;
                    moved = true;
                }
                if map.is_area_passable(units[j].2.disc_at(new_b)) {
                    units[j].1 = new_b;
                    moved = true;
                }
            }
    ...
    for (e, pos, _) in units {
        if let Ok(mut p) = ecs.get::<&mut Position>(e) {
            *p = pos;
        }
    }
}
